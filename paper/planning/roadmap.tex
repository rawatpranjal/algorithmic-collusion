...
\begin{itemize}
    \item \textbf{Theoretical Benchmarks vs. Practical Realities:} Auction theory provides insights into optimal mechanisms under simplified assumptions (e.g., truthful bidding in SPA \cite{Vickrey1961}, revenue equivalence under certain conditions). For more dynamic settings with constraints, concepts like Price of Anarchy (PoA) offer worst-case bounds \cite{DengVarious}. However, these often represent lower (PoA) or upper (optimal welfare/revenue) bounds, leaving the vast space of average-case, practical performance unexplored when agents are actively learning and adapting.
    \item \textbf{Algorithmic Collusion and Emergent Behavior:} While some research (including prior work by the author \cite{Rawat2025AlgorithmicCollusion}) has explored how Q-learning agents might lead to collusive outcomes in auctions, the behavior of simpler, more industry-relevant pacing algorithms (like Dual Gradient, UCB, or MWU for pacing multipliers) in this context is less understood. Phenomena like "spontaneous coupling" \cite{BanchioMantegazza2022} highlight that even with distinct objectives, learning agents can inadvertently synchronize behavior, potentially leading to sub-optimal system outcomes. This study investigates if similar emergent dynamics arise from pacing learners.
    \item \textbf{Focus on Equilibrium vs. Learning Dynamics:} Much of the auto-bidding literature focuses on equilibrium properties (e.g., First-Price Pacing Equilibrium (FPPE), Second-Price Pacing Equilibrium (SPPE) \cite{Conitzer2021SPPE, Conitzer2022FPPE}) or single-agent optimization problems \cite{Aggarwal2019}. There is less emphasis on the transient learning dynamics, the stability of these learned states, and how different auction designs influence the path to (or oscillations around) such states.
\end{itemize}
This research aims to fill this gap by providing a systematic, simulation-based evaluation of auction design robustness when confronted with practical learning-based auto-bidding.

\subsection{Research Questions and Objectives}
The primary goal of this research is to determine which auction design (focusing on FPA vs. SPA) offers better practical robustness and performance for a platform when advertisers use common online learning algorithms for uniform pacing under RoS and budget constraints.
\begin{itemize}
    \item \textbf{Primary Research Question:} Which auction design (FPA vs. SPA) is practically superior for a platform—prioritizing platform revenue, then social welfare and system stability—when advertisers employ common online learning algorithms for pacing?
    \item \textbf{Secondary Research Questions:}
        \begin{enumerate}
            \item How do different learning algorithms (e.g., Dual Gradient, UCB, MWU) interact with FPA and SPA, and does the optimal auction design depend on the prevalent learning mechanism?
            \item How do market structure (number of bidders, valuation distributions) and advertiser constraints (RoS targets, budget tightness) modulate the relative performance and stability of FPA and SPA?
            \item What is the role of reserve price policies in enhancing the performance or mitigating the risks associated with FPA and SPA in these learning environments?
            \item Can we identify conditions under which FPA or SPA are more susceptible to emergent undesirable behaviors, such as tacit collusion (manifesting as persistently low pacing multipliers) or system instability ("bidding crashes")?
        \end{enumerate}
\end{itemize}

\textbf{Objectives:}
\begin{enumerate}
    \item To develop a flexible simulation framework modeling a canonical auto-bidding environment where agents learn a single pacing multiplier.
...
\begin{enumerate}
    ...
    \item Conducting a systematic experimental design, varying auction mechanisms, learning algorithms, market covariates, and reserve price policies across a grid of parameters (Section \ref{sec:experimental_design}).
...
\begin{enumerate}
    \item \textbf{Bridging Theory and Practice:} Providing insights into the average-case, practical performance of FPA and SPA in learning-driven auto-bidding, moving beyond theoretical optima or worst-case bounds.
    ...
    \item \textbf{Understanding Emergent Dynamics with Pacing Learners:} Investigating how simpler, industry-relevant pacing algorithms (as opposed to full RL agents) can lead to complex system dynamics, including potential collusion-like states or instabilities, and how auction design influences these.
    ...
    \item \textbf{Methodological Framework:} Providing a detailed simulation framework that can be extended by other researchers to explore further complexities in auto-bidding systems.
\end{enumerate}

\section{Literature Review (Brief and Focused)}
This research builds upon several streams of literature:
\begin{itemize}
    \item \textbf{Auction Theory:} Foundational works by Vickrey \cite{Vickrey1961}, Myerson \cite{Myerson1981}, Clarke \cite{Clarke1971}, and Groves \cite{Groves1973} establish core concepts of auction design and efficiency.
    \item \textbf{Auto-bidding and Pacing Equilibria:} Work by Aggarwal et al. \cite{Aggarwal2019, Aggarwal2024Survey} details optimal bidding for constrained agents. Conitzer et al. \cite{Conitzer2021SPPE, Conitzer2022FPPE} define and analyze FPPE and SPPE, providing equilibrium benchmarks. Deng et al. (various) explore efficiency and revenue in auto-bidding, often with uniform bidding assumptions. These provide the theoretical backdrop for what converged, rational agents might achieve.
    \item \textbf{Algorithmic Collusion and Multi-Agent Learning:} Research by Banchio \& Mantegazza \cite{BanchioMantegazza2022} on "spontaneous coupling" and the author's prior work on Q-learning in auctions \cite{Rawat2025AlgorithmicCollusion} highlight the potential for learning agents to reach unintended synchronized states. This proposal extends this line of inquiry to simpler pacing algorithms.
    \item \textbf{Simulation Studies in Algorithmic Markets:} Various studies use simulation to explore algorithmic pricing or bidding, but few systematically compare FPA vs. SPA with a focus on the learning dynamics of practical pacing algorithms across such a broad range of metrics and covariates.
\end{itemize}
This proposal directly addresses the need for more empirical, simulation-based evidence on the \textit{dynamic and emergent properties} of auction markets populated by auto-bidders using practical learning strategies for pacing.
...
Each advertiser $i$ is an auto-bidder agent aiming to maximize their utility (e.g., net surplus or volume of valuable impressions) subject to performance constraints.
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Dual Gradient-Descent Style (Primary Algorithm):}
        \begin{itemize}
            ...
            \item \textit{Error Signal:} Primarily driven by RoS deviation. A common formulation for the error (or gradient component) related to RoS is $e_{\text{RoS},i} = P_i^W \tau_i - V_i^W$. If $e_{\text{RoS},i} > 0$, the agent is underspending relative to its value at the target RoS (or RoS is too low), suggesting $\alpha_i$ might need to increase (to win more/pay more if needed). If $e_{\text{RoS},i} < 0$, RoS is too high, suggesting $\alpha_i$ might decrease. (The exact sign and update direction needs careful formulation to ensure convergence to the target). If a budget constraint $B_i^{\text{period}}$ for the window is active, an additional error term $e_{\text{Budget},i} = P_i^W - B_i^{\text{period}}$ can be added.
            \item \textit{Update Rule:} $\alpha_i(t+1) = \text{clip}(\alpha_i(t) - \eta \cdot (\lambda_{\text{RoS}} e_{\text{RoS},i} + \lambda_{\text{Budget}} e_{\text{Budget},i}), \alpha_{\min}, \alpha_{\max})$. Learning rate $\eta$ and weights $\lambda$ are key parameters.
        \end{itemize}
    \item \textbf{Bandit Algorithms (e.g., UCB1):}
        \begin{itemize}
            \item \textit{Arms:} $K_A$ discretized levels of $\alpha_i$ within $[\alpha_{\min}, \alpha_{\max}]$.
            \item \textit{Reward (after window $W$ playing arm $\alpha_i^k$):} $R_i^k = V_i^W$ (if RoS and Budget constraints are met) minus penalties for constraint violations. For example, $R_i^k = V_i^W - \text{penalty}_{\text{RoS}} \cdot \max(0, \tau_i P_i^W - V_i^W) - \text{penalty}_{\text{Budget}} \cdot \max(0, P_i^W - B_i^{\text{period}})$.
    ...
    \item \textbf{Multiplicative Weights Update (MWU):}
        \begin{itemize}
            ...
            \item \textit{Loss (for expert $\alpha_i^k$ after window $W$):} $L_i^k = -V_i^W + \text{LargePenaltyIfRoSViolated}(\alpha_i^k) + \text{LargePenaltyIfBudgetViolated}(\alpha_i^k)$. Loss is inversely related to value achieved, plus large penalties for constraint misses.
            \item \textit{Update Rule:} $w_k(t+1) = w_k(t) \cdot \exp(-\eta \cdot L_i^k)$, followed by normalization of weights $w_k$.
        \end{itemize}
\end{itemize}
(Detailed mathematical formulations for each algorithm, including handling of edge cases like zero spend, will be provided in Appendix \ref{app:learning_algo_details}.)
...
\begin{itemize}
    ...
    \item \textbf{Reserve Price Policy (Secondary Design Lever):}
        \begin{enumerate}
            ...
            \item \textbf{Static Reserve Price:} A fixed reserve $r$ applied to all auctions. This $r$ will be set as a percentage (e.g., 0\%, 25\%, 50\%) of a characteristic of the $v_{i,j}$ distribution (e.g., mean E$[v_{i,j}]$ or a specific quantile). If all bids are below $r$, no sale occurs.
        \end{enumerate}
\end{itemize}
...
\begin{table}[h!]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        \textbf{Auto-bidder Setup} & RoS Target $\tau_i$ & \{Homogeneous (e.g., 1.5, 2.5, 4.0); \\
         & & Heterogeneous (e.g., drawn from $U[1.5, 4.5]$)\} \\
         & Budget $B_i$ & \{RoS-only; Tight (e.g., 50\% of potential RoS-spend); \\
         & & Loose (e.g., 150\% of potential RoS-spend)\} \\ \midrule
        \textbf{Market Structure} & Num Advertisers $N$ & \{3, 5, 10, 20\} \\
         & Num Auctions $M$ per run & \{$10^5$, $5 \times 10^5$\} (ensure learning convergence) \\
         & Advertiser $v_{i,j}$ Dist. & \{Unif[0.5,1.5]; LogNormal($\mu_v$=1, $\sigma_v^2 \in \{0.25, 1.0\}$)\} \\ \midrule
...
\begin{itemize}
    ...
    \item \textbf{System Stability \& Learning Dynamics:}
        \begin{itemize}
            ...
            \item Steady-State Pacing Multipliers: Average and distribution of $\alpha_i(t)$ in the post-convergence phase.
            \item Pacing Multiplier Volatility/Oscillation: StdDev of individual $\alpha_i(t)$ in steady state.
            \item "Crash" Metrics: Frequency and magnitude of sharp drops in platform revenue or average $\alpha_i(t)$; variance of first differences of these series.
...
\begin{enumerate}
    ...
    \item \textbf{Interaction Effects (Heterogeneity Analysis):} This is crucial. ANOVA-like approaches or regression models (e.g., using Double Machine Learning if complex non-linearities are suspected, similar to \cite{Rawat2025AlgorithmicCollusion}, though simpler models may suffice) will be used to identify how the treatment effect of auction design (FPA vs. SPA) changes with covariates (market structure, advertiser constraints, learning algorithm type).
        ...
        \end{itemize}
    \item \textbf{Analysis of Learning Dynamics:} Time-series plots of average $\alpha_i(t)$, revenue, etc., to visualize convergence, stability, and potential oscillations or "collusion-like" states (e.g., persistently low $\alpha_i(t)$ in FPA despite RoS headroom). Spectral analysis might be used if strong periodicities are observed.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Revenue and Welfare:} SPA might generally yield higher revenue and welfare due to less aggressive bid shading by learning agents, but this could be context-dependent. FPA might lead to lower pacing multipliers on average.
    \item \textbf{Stability:} SPA might exhibit more stable revenue and agent behavior, while FPA could be more prone to "boom-bust" cycles or convergence to multiple equilibria, some of which might be low-revenue ("collusion-like").
...
\begin{itemize}
    \item \textbf{Market Structure:} We hypothesize that FPA's susceptibility to low-revenue outcomes (due to learned bid shading via low $\alpha_i(t)$) might be exacerbated in markets with fewer bidders ($N$) or less valuation variance. SPA might be more robust to these changes.
    ...
    \item \textbf{Reserve Prices:} Optimal static reserve prices are expected to improve revenue for both FPA and SPA, potentially by truncating the action space and mitigating extreme bid shading in FPA. The interaction between reserve prices and the learning dynamics is a key point of investigation.
\end{itemize}
...
\begin{itemize}
    ...
    \item We will specifically look for evidence of FPA leading to persistently lower average $\alpha_i(t)$ values compared to SPA, even when RoS targets might allow for higher bids, and how this is affected by market conditions and learning algorithms. This will be framed in the context of emergent "collusion-like" shading.
\end{itemize}

\section{Timeline and Resources}
\begin{itemize}
    \item \textbf{Phase 1 (Months 1-2):} Refine simulation framework, implement core auction mechanisms (FPA, SPA) and one primary learning algorithm (e.g., Dual Gradient). Initial pilot runs and debugging.
    \item \textbf{Phase 2 (Months 3-4):} Implement remaining learning algorithms (UCB, MWU). Develop and test the full suite of outcome metrics and analysis scripts.
    \item \textbf{Phase 3 (Months 5-7):} Conduct full-scale simulation runs according to the experimental grid. Data collection and initial processing.
    \item \textbf{Phase 4 (Months 8-10):} In-depth data analysis, heterogeneity analysis, interpretation of results.
    \item \textbf{Phase 5 (Months 11-12):} Manuscript preparation, drafting of discussion and conclusions, revisions.
\end{itemize}
\textbf{Resources:} Access to sufficient computational resources (e.g., university HPC cluster or cloud computing) will be required for the extensive simulation runs. Standard statistical software (R, Python) will be used for data analysis.

\section{Conclusion and Broader Impact}
This research will provide a much-needed empirical evaluation of auction design choices in the context of modern, learning-driven auto-bidding. By moving beyond static equilibrium analysis and focusing on the dynamic interactions of adaptive agents, this study will offer significant practical insights for advertising platforms. The findings will help platforms make more informed decisions about which auction mechanisms and reserve price policies are most robust and effective, ultimately leading to more efficient, stable, and fair advertising marketplaces. The methodological framework developed will also serve as a valuable tool for future investigations into the complex world of algorithmic markets.
