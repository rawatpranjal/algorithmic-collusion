...
The comparison between multiplicative dual pacing and PI control isolates the role of the control mechanism from the budget constraint itself. In both experiments, the budget multiplier is the dominant revenue driver ($|t| = \ExpFourARevFactorTOne$ in Experiment~4a; $|t| = 31.2$ in Experiment~4b), confirming that budget tightness, not the pacing algorithm, is the primary determinant of revenue outcomes. The number of bidders ranks among the top factors in both cases ($|t| = \ExpFourARevNbidAbsT$ in 4a; $|t| = 16.1$ in 4b), while auction format plays a stronger role under PI control ($|t| = 11.0$) than under dual pacing ($|t| = \ExpFourARevAuctionT$). All PoA values remain well within the theoretical worst-case bound of~2 across both pacing mechanisms. The PI controller's additive updates and cumulative error tracking produce different bid-to-value ratio levels than multiplicative pacing, but the factor hierarchy is consistent across both approaches, indicating that the structural features of the budget-constrained environment dominate the choice of control algorithm.
