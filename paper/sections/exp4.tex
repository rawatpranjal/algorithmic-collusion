...
HC3 robust standard errors (Section~L of the full robustness analysis in \texttt{results/exp4/robust/}) confirm significance of all main effects and the algorithm $\times$ budget tightness interaction. Holm-Bonferroni corrections across all tests leave the primary effects intact. LASSO variable selection retains budget tightness, algorithm, and auction type in all responses, corroborating the Pareto chart rankings. The model $R^2$ is high across all responses (0.85--0.96), indicating that the nine-factor design captures the dominant sources of variation with minimal residual confounding.
