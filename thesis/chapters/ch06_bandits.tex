...
Experiment~2a uses the $3 \times 2^7$ mixed-level factorial from Chapter~\ref{ch:design_and_inference}, with 384 cells replicated twice for \ExpTwoANobs{} observations. \Cref{tab:exp2_params_summary} summarises the factors.
...
The number of bidders dominates ($|t| = \ExpTwoARevFactorTOne$), producing its largest effect of any experiment in the thesis. The shift from two to four bidders raises revenue by \ExpTwoARevTopPctEffect{} percent of the grand mean, roughly \ExpTwoARevTopVsAuctionRatio{} times the effect of auction format. The full effect hierarchy appears in \Cref{tab:exp2a_ranked_rev} (Appendix~\ref{ch:ranked_tables}).
...
Tuning parameters are essentially inert. Regularisation $\lambda$ and memory decay $\delta$ both fail to produce significant main effects on revenue, consistent with the Sobol decomposition in Appendix~\ref{ch:robustness}, where algorithmic parameters collectively explain less than four percent of revenue variance.
...
The number of bidders is again the top factor ($|t| = \ExpTwoBRevFactorTOne$), followed by reserve price ($|t| = \ExpTwoBRevFactorTTwo$). The full effect hierarchy appears in \Cref{tab:exp2b_ranked_rev} (Appendix~\ref{ch:ranked_tables}).
...
Unlike LinUCB, Thompson Sampling produces a more diffuse factor hierarchy. No single factor accounts for the lion's share of revenue variance, and several factors contribute in similar measure. Robustness checks in Appendix~\ref{ch:robustness} confirm these rankings.
