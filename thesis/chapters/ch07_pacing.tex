...
The factorial model for effective Price of Anarchy achieves $R^2 = \ExpThreeAPoaRsq$. The significant effects are listed in \Cref{tab:exp3a_ranked_poa} (Appendix~\ref{ch:ranked_tables}).
...
Robustness checks in Appendix~\ref{ch:robustness} confirm these findings across all thirteen diagnostic tests.
...
The budget multiplier again dominates with $|t| = \ExpThreeBRevFactorTOne$ and an effect of \ExpThreeBRevTopPctEffect{} percent of the grand mean. The full effect hierarchy appears in \Cref{tab:exp3b_ranked_rev} (Appendix~\ref{ch:ranked_tables}).
