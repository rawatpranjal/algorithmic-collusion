...
This chapter describes the auction environment shared by all experiments and the budget-constrained pacing algorithms specific to Experiments~3a and~3b. The Q-learning and contextual bandit algorithms used in Experiments~1a through~2b are defined in Chapter~3 of this thesis;\footnote{Chapter~3 covers the formal update rules for Q-learning (synchronous and asynchronous Bellman updates), the exploration strategies ($\varepsilon$-greedy, Boltzmann, upper confidence bound, Thompson Sampling), and the convergence properties of each algorithm family.} this chapter summarises only the features relevant to the experimental designs. Equilibrium benchmarks are stated briefly, with derivations in Appendix~\ref{ch:equilibria}.

\section{Auction Environment}
\label{sec:env}

The environment is a sequence of single-item auctions indexed by $t = 1, 2, \ldots, T$, with $n$ bidders competing in each round. In round~$t$, bidder~$i$ submits a bid $b_{it}$ from a finite grid $\mathcal{B} = \{0, 0.1, 0.2, \ldots, 1.0\}$ and pays a price determined by the auction rule.\footnote{The eleven-point grid is fine enough to let distinct bidding behaviours emerge yet coarse enough for full exploration. Robustness checks in Appendix~\ref{ch:robustness} confirm that factor rankings are stable across grid sizes of 6, 11, and 21.}
