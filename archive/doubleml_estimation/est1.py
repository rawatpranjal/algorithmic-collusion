...
analysis_log_path = os.path.join("results/exp1", "analysis_stdout.txt")
...
with open(analysis_log_path, "w") as f:
    ...
    # ---------------------------------------------------------------------
    df = pd.read_csv("results/exp1/data.csv")  # Modify path if needed
    ...
    # ---------------------------------------------------------------------
    gate_folder = os.path.join("results/exp1", "gate_plots")
    cate_folder = os.path.join("results/exp1", "cate_plots")
    ...
    for outcome in outcomes_list:
        ...
        if len(cont_covs) > 0:
            ...
            for i, cvar in enumerate(cont_covs):
                ...
                ax_single.plot(x_sort, eff_sort, label='CATE')
                ax_single.fill_between(x_sort, lo_sort, hi_sort, alpha=0.2, label='95% CI')
    ...
    with open("results/exp1/estimation_results.json", "w") as jf:
        json.dump(structured, jf, indent=2, default=str)
    print("Structured results saved to results/exp1/estimation_results.json")
