...
def main():
    ...
    for outcome in outcomes_list:
        ...
        if len(cont_covs) > 0:
            ...
            for i, cc in enumerate(cont_covs):
                ...
                ax_single.set_title(f"CATE: {cc} ({outcome})")
                ax_single.set_xlabel(cc)
                ax_single.set_ylabel("Estimated TE")
                ax_single.legend()
                fig_single.tight_layout()
                fig_single.savefig(os.path.join(cate_folder, f"cate_{outcome}_{cc}.png"),
    ...
    print("Structured results saved to results/exp3/estimation_results.json")
